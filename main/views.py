...
from .models import PDFDocument, SummaryNotes, Video, Article
from .forms import PDFUploadForm, SummaryForm, TextForm, YouTubeForm, ArticleForm
from main.serializers import FlashcardSerializer

from issak.utils.helpers import (
...
class PDFMindMapView(View):
    def post(self, request):
        text = request.session.get('pdf_text')
        if not text:
            return JsonResponse({'error': 'No PDF text found in session'}, status=400)
        
        try:
            json_data = generate_mindmap_from_summary(text)
            if "error" in json_data:
                return JsonResponse({'error': json_data['error']}, status=400)
                
            return JsonResponse({'mindmap': json_data}, status=200)
